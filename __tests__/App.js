...
import RNAwsSigner from 'react-native-aws-signer'

import * as Keychain from 'react-native-keychain'

const RNETHKIT_APIKEY = 'eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJhcGlfY2xpZW50X2lkIjoiNGVkNTNiYTAtNTRjYy00M2QwLTk4MDgtZGZiMTY2ZDhhMmI4IiwiY3JlYXRlZF9hdCI6MTUwNzIzNjQ4OH0.z4_h_4iTCYyv0OMCqe6RE0XEvM_DIagTR3lfRbQt74w' // local
...
it('configures rn ethereum kit correctly', () => {
  RNEthereumKit.init(RNETHKIT_APIKEY)
  expect(global.rnEthKitConfig.apiKey).toBe(RNETHKIT_APIKEY)
  expect(global.rnEthKitConfig.sdkdHost).toBeTruthy()
})

it('tests ethwallet with recovery phrase', async () => {
  // clean keychain
  ...
  RNEthereumKit.init(RNETHKIT_APIKEY)
  let w = new ETHWallet({debug: false})
  expect(w).toBeTruthy()
  ...
  // mock sendTx response
  fetch.mockResponseOnce(JSON.stringify({'jsonrpc':'2.0','result':'0xbeef123','id':'91d4bdce10ded70e797cdef8d1904a4e'}))

  // send to random eth address
  let txHash = await w.sendTx('0x9899AF5Aa1EfA90921d686212c87e70F4fbea035', 100)
  expect(txHash).toBe('0xbeef123')
})

it('tests ethwallet with default options', async () => {
  // clean keychain
  Keychain.setInternetCredentials('sdkd_private_key_for_test@example.com', null, null)
...
it('tests ethwallet email recovery via QR code', async (done) => {
  // clean keychain
  Keychain.setInternetCredentials('sdkd_private_key_for_test@example.com', null, null)
...
it('tests rn-shamirs-sss splitting into parts', async () => {
...
it('tests rn-shamirs-sss combining parts', async () => {
  let secret = 'd94527908e99bcff99bf7106f16d2490cf60e692'
  let shares = [
...
it('tests rn-aws-ses email sending', async (done) => {
...
it('tests rn-aws-signer signature generation', async () => {
  ...
  let signer = new RNAwsSigner(signingConfig)
